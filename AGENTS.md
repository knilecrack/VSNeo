...
The which-key hints (`WhichKeyPopup.cs`) are the companion's mapping table rendered Visual Studio-side: after the rc loads, Lua pushes `vsneo_keymaps` (`n` and `x`, `[lhs, desc]` pairs, leader already expanded, `<Plug>` filtered out), and the key processor matches every sent key against the cached table. Only a sequence that is a strict prefix of a mapping arms the 450 ms delay — a fast typist never sees the popup; a pause after `<leader>` does. Token comparison normalizes `<...>` casing (nvim says `<C-W>`, `KeyEncoder` says `<C-w>`) and folds `<Space>` to a plain space. The push is one-shot: nvim has no mapping-added event, so mappings created later (lazy `:packadd`) stay invisible until restart, and buffer-local mappings are out of scope.
