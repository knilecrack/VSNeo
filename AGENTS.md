...
- Insert mode passes through to Visual Studio untouched so IntelliSense, snippets, and brace completion keep working. Only `<Esc>` is claimed in insert. The cost is that nvim's keystroke-based redo record never sees typed text — the companion reconstructs `.` itself for changes that pass through insert (see the dot-repeat section at the bottom of `Lua/vsneo.lua`); macros still replay without typed text.
